...
class Metrics(object):
    ...
    def compared_tag(self,input,pre_label,std_label): #对比预测结果和实际结果，输出与实际结果不同的预测结果

        compared_char_metrix=np.array(self.predict_tags)!=np.array(self.std_tags)
        if np.count_nonzero(compared_char_metrix)!=0:
            output=(input,np.array(pre_label),np.array(std_label))
            print(output,'///////////////////')
    ...
    #计算每类实体的准确率
        ...
        for entity in self.entity_set:                                                              #这里计算结果可能有问题
            if entity not in self.predict_entity_counter:
                self.predict_entity_counter[entity]=0
