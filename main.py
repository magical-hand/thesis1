...
def train(**kwargs):
    ...
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer_real, 0.1, last_epoch=-1)
    eval_loss = 10000
    ...
    for epoch in range(config.base_epoch):
        ...
        for batch in train_loader:
            ...
            # try:
            #     plt.plot(total_step_list, loss_list, '-b')
            # except:
            #     plt.plot(total_step_list,loss_list)
            # plt.pause(0.01)
            # draw_time=time.time()
            # logging.info('draw_time {}'.format(draw_time-forward_time))
...
def arch_train(dev_loader,architect):
    ...
    architect.step(input_valid=input_search,masks_valid=masks_search,target_valid=target_search)
